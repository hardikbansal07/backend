...
def test_startup_succeeds_even_when_world_city_file_missing(monkeypatch, tmp_path):
    service._reset_world_city_index_cache_for_tests()
    ...
    # Logic should NOT raise FileNotFoundError now
    with TestClient(api_app.app) as client:
        response = client.get('/api/places?q=test')
