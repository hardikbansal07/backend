...
def run_calc():
    ...
    print("\n" + "="*60)
    print("      SAPTAVARGAJA BALA BREAKDOWN FOR ALL PLANETS (D1 to D30)")
    print("="*60)
    ...
    for idx, name in enumerate(p_names):
        ...
        for div in divisions:
            ...
            # Temporary relationship is calculated based on signs in D1
            ...
            nat = rel_matrix.get_natural_relationship(name, sign_lord)
            temp = rel_matrix.get_temporary_relationship(planet_sign, lord_sign)
            comb = rel_matrix.get_combined_relationship(nat, temp)
            
            if rel_matrix.is_moolatrikona_sign(name, v_sign): status = "Moolatrikona"
            elif rel_matrix.is_own_sign(name, v_sign): status = "Own Sign"
            else: status = comb
            
            details.append(f"  D{div:2d} in {v_sign:11} (Lord: {sign_lord:7}) -> {status:10} = {score:6.2f}")
            
        for det in details:
            print(det)
        print(f"  -------------------------------------------------------------")
        print(f"  TOTAL SAPTAVARGAJA BALA = {total_score:.2f} Shashtiamsas")
